...
def detail_ticket(request, ticket_id):
    ...
    # Flags utilisés dans le template
    ...
    # Messages liés au ticket
    messages_ticket = Message.objects.filter(ticket=ticket).order_by('date_envoi')

    # Gestion du formulaire d'ajout de message
    if request.method == "POST":
        form = FormulaireMessage(request.POST)
        if form.is_valid():
            # Empêche l'ajout si le ticket est résolu
            if ticket.statut == 'resolu':
                messages.error(request, "Impossible d'ajouter un message sur un ticket résolu.")
            else:
                m = form.save(commit=False)
                m.ticket = ticket
                m.auteur = request.user
                m.save()
